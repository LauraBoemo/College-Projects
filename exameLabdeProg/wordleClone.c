...
void jogando_wordle(FILE *entrada, FILE *saida) {
    chute_t chute;
    partida_t partida;
    palavra_t palavra;
...
int jogando_wordle(FILE *entrada, FILE *saida) {
    chute_t chute;
    partida_t partida;
    palavra_t palavra;
...
/*-----------------------------------------------------------------------------*/
/* Corpo da função - tela_secundaria                                           */
/*                                                                             */
/* Função que desenha na tela o teclado conforme suas devidas cores            */
/* indicadas.                                                                  */
/*-----------------------------------------------------------------------------*/

void tela_secundaria(chute_t *chute, partida_t *partida) {
    int aux = 0, aux_letras = 0;
    char letras_corretas[4]; 
        
    for(char a='a'; a!='z'; a++) {
        for(int aux2=0; aux2<5; aux2++) {
            if(partida->chutes[partida->nchutes].palavra.sem_acento[aux2] == a) {
                if(chute->correto) {
                    partida->letras_usadas[aux] = partida->chutes[partida->nchutes].dicas[aux2];
                } else if(partida->chutes[partida->nchutes].dicas[aux2] == chuteV) {
                    letras_corretas[aux_letras] = partida->chutes[partida->nchutes].dicas[aux2];
                    partida->letras_usadas[aux] = partida->chutes[partida->nchutes].dicas[aux2];
                    aux_letras++;
                } else if(partida->chutes[partida->nchutes].dicas[aux2] == chuteL) {
                    for(int aux3 = 0; aux3 < 4; aux3++) {
                        int cont = 0;
                        if(letras_corretas[aux3] != a) {
                            cont++;
                            if(cont == 4) {
                                partida->letras_usadas[aux] = partida->chutes[partida->nchutes].dicas[aux2];
                            }
                    ...
                    partida->letras_usadas[aux] = chuteC;
                }
            }
        }

        switch(partida->letras_usadas[aux]) {
            case chuteC:
                muda_cor(BRANCO, CINZA);
            break;
            case chuteL:
...
bool le_palavra(FILE *arquivo) {
    int i = 0;
    while (fscanf(arquivo, "%s", lista_de_palavras[i].sem_acento) == 2) ++i;
    return true;
}
...
bool le_palavra(FILE *arquivo) {
    ...
    while (fscanf(arquivo, "%s", lista_de_palavras[i].sem_acento) == 1) ++i;
    return true;
}
